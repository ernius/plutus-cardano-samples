...
import           Ledger                    (Address, Validator, ScriptContext, Value, scriptAddress, getCardanoTxId, ChainIndexTxOut(..), dataHash, Datum (..), DatumHash (..))
...
solution (SolutionParams theProposal) = do
    ...
    unspentOutputs <- Map.filter hasCorrectDatum <$> utxosAt bountyAddress
    ...
    logInfo @String $ printf "proposed solution is %d" theProposal
      where
        ...
        hasCorrectDatum (ScriptChainIndexTxOut _ _ (Right (Datum datum)) _)    =
          ...
          Just d  -> theProposal * theProposal == d
          Nothing -> False
        hasCorrectDatum (ScriptChainIndexTxOut _ _ (Left (DatumHash datumHash)) _) =
          (dataHash $ PlutusTx.toBuiltinData (theProposal * theProposal)) == datumHash
