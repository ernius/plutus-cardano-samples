...
test = runEmulatorTraceIO' def emCfg myTrace
  where
    ...
    emCfg = EmulatorConfig (Left $ Map.fromList $ [(knownWallet 1, vOracle)] <> [(knownWallet i, v) | i <- [2 .. 5]]) def def
...
myTrace = do
    ...
    callEndpoint @"use" h4 20
    void $ Emulator.waitNSlots 1

    -- callEndpoint @"update" h1 2
    -- void $ Emulator.waitNSlots 3

    -- callEndpoint @"use" h5 10
    -- void $ Emulator.waitNSlots 3
